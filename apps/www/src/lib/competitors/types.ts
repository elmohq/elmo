...
export function getScreenshotUrl(url: string): string {
	return `https://api.screenshotone.com/take?url=${encodeURIComponent(url)}&format=jpg&block_ads=true&block_cookie_banners=true&block_trackers=true&delay=0&timeout=60&response_type=by_format&image_quality=80`;
}
