...
export function Features() {
	...
	return (
		<section id="features" className="border-y border-[#e3e9f5] bg-white">
			<div className="mx-auto max-w-6xl px-4 py-20 md:px-6 lg:py-24">
				<SectionHeading
					eyebrow="Inside Elmo"
					title="Every number, one click from the answer behind it."
					lede="The loop above is the product. Measure where you show up, understand why, and act on it, from a single prompt to a year of trends."
				/>
				...
				<div className="mt-12 grid gap-10 lg:mt-16 lg:grid-cols-12 lg:items-start lg:gap-12">
					<div className="lg:col-span-4">
						{groups.map((group) => (
							<div key={group.name} className="mb-5 last:mb-0">
								<p className="mb-2 px-3 text-[12px] font-medium text-slate-400">{group.name}</p>
								<ul className="space-y-0.5">
									{group.features.map((f) => {
										...
										return (
											<li
												key={f.id}
												className={`rounded-lg transition-colors ${active ? "bg-[#f3f6fc] shadow-[inset_0_0_0_1px_rgb(30_58_138/0.07)]" : "hover:bg-[#f3f6fc]/70"}`}
											>
												<button
													type="button"
												...
												>
													<Icon
														...
														aria-hidden="true"
													/>
													<span className={`text-[15px] font-medium ${active ? "text-slate-950" : "text-slate-600"}`}>
														{f.label}
													</span>
												</button>
												<div
												...
												>
													<div className="overflow-hidden">
														<div className="px-3 pb-4 pl-10">
															<h3 className="text-[15px] font-semibold leading-snug text-slate-950">{f.title}</h3>
															<p className="mt-1.5 text-pretty text-sm/6 text-slate-600">{f.description}</p>
															{active ? (
					...
					<div className="sticky top-24 hidden lg:col-span-8 lg:block">
						<div className="rounded-3xl bg-[#f3f6fc] p-6 shadow-[inset_0_0_0_1px_rgb(30_58_138/0.05)]">
							{all.map((f) => (
