	...
	return (
		<footer className="border-t border-white/10 bg-black">
			<div className="mx-auto max-w-6xl px-4 py-12 md:px-6">
				<div className="grid gap-10 md:grid-cols-12">
					<div className="md:col-span-3">
						<Link to="/" aria-label="Homepage">
							<Logo className="text-3xl text-blue-500" />
						</Link>
						<p className="mt-5 max-w-[36ch] text-pretty text-sm/6 text-zinc-400">
							Elmo is the #1 open source AEO platform. Track and improve how AI answers talk about your brand.
						</p>
						<div className="mt-5 inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 font-mono text-[11px] text-zinc-300">
								...
								<ul className="mt-4 space-y-2.5 text-sm text-zinc-400">
									{col.links.map((link) =>
										"external" in link && link.external ? (
											<li key={link.href}>
												<a href={link.href} target="_blank" rel={externalRel(link.href)} className="hover:text-white">
													{link.label}
												</a>
											</li>
				...
				<div className="mt-12 flex flex-col items-start gap-4 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
					<p className="font-mono text-[11px] text-zinc-500">
						&copy; {new Date().getFullYear()}{" "}
						<a href={BLUEWHALE_URL} target="_blank" rel={externalRel(BLUEWHALE_URL)} className="hover:text-zinc-300">
							Blue Whale Software, LLC
						</a>
					</p>
					<div className="flex items-center gap-3 text-zinc-500">
						<a href="https://x.com/tryelmo" target="_blank" rel="noopener noreferrer" className="hover:text-white">
