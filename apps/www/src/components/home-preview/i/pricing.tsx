...
const SECONDARY = `${BTN} bg-white text-zinc-900 ring-1 ring-zinc-200 hover:bg-zinc-50 hover:ring-zinc-300`;
...
function PlanCta({ plan }: { plan: Plan }) {
	...
	return (
		<div className="[&>button]:!h-9 [&>button]:w-full [&>button]:rounded-md [&>button]:bg-white [&>button]:!text-sm [&>button]:font-medium [&>button]:text-zinc-900 [&>button]:ring-1 [&>button]:ring-zinc-200 [&>button]:hover:bg-zinc-50 [&>button]:hover:ring-zinc-300">
						...
						<div
							...
							className={`relative flex flex-col rounded-2xl p-7 ${
								plan.featured
									...
									: "bg-white shadow-[0_0_0_1px_rgb(24_24_27/0.08),0_1px_2px_rgb(24_24_27/0.04)]"
							}`}
						>
							{plan.featured ? (
							...
							<h3 className="text-lg font-semibold tracking-tight text-zinc-950">{plan.name}</h3>
							<p className="mt-1 text-sm text-zinc-600">{plan.desc}</p>
							<div className="mt-6 flex items-baseline gap-1.5">
								<span className="text-4xl font-semibold tracking-[-0.03em] text-zinc-950 tabular-nums">
									{plan.price}
								</span>
								{plan.priceLabel ? <span className="text-sm text-zinc-500">{plan.priceLabel}</span> : null}
							</div>
							<div className="mt-6">
								<PlanCta plan={plan} />
							</div>
							<ul className="mt-7 space-y-3 border-t border-zinc-100 pt-6 text-sm text-zinc-700">
								{plan.features.map((f) => (
									<li key={f} className="flex items-start gap-2.5">
										<Check
											className={`mt-0.5 size-4 shrink-0 ${plan.featured ? "text-blue-600" : "text-zinc-400"}`}
											strokeWidth={2.5}
											aria-hidden="true"
										/>
										<span>{f}</span>
