// Copied from the shared Stats section so this variant can lay the model list out its own way.
			...
			<div className="mx-auto max-w-6xl px-4 py-16 md:px-6 lg:py-20">
				<div className="grid gap-4 lg:grid-cols-12 lg:items-end lg:gap-12">
					<div className="lg:col-span-7">
						<Eyebrow>Model coverage</Eyebrow>
						<h2 className="mt-4 text-[2rem] font-semibold leading-[1.1] tracking-[-0.025em] text-balance text-zinc-950 md:text-[2.75rem]">
							Your buyers don't only ask ChatGPT.
						</h2>
					</div>
					<p className="max-w-[52ch] text-pretty text-base/7 text-zinc-600 lg:col-span-5 lg:pb-1.5">
						Elmo asks your prompts on every major AI model. See what ChatGPT users see via web scraping, or track
						responses through each model's API or OpenRouter with your own keys.
					</p>
				</div>
				<ul className="mt-10 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-5 md:gap-3">
					{MODELS.map((m) => {
						const Icon = m.icon;
						return (
							<li
								key={m.name}
								className="flex h-12 items-center gap-3 rounded-xl md:h-14 bg-white px-4 shadow-[0_0_0_1px_rgb(24_24_27/0.07),0_1px_2px_rgb(24_24_27/0.04)]"
							>
								<span className="size-[18px] shrink-0 text-zinc-800">
									<Icon />
								</span>
								<span className="text-[13px] font-medium leading-tight text-zinc-800 md:text-sm">{m.name}</span>
							</li>
						);
					})}
				</ul>
				<p className="mt-5 text-center font-mono text-[11px] uppercase tracking-[0.18em] text-zinc-500">
					+ the rest, via OpenRouter
				</p>
			</div>
		</section>
	);
