...
function organizationGroup(organization: OrganizationSummary, features?: FeaturesConfig): NavGroup {
	...
	if (features?.billing) {
		items.push({
			title: /* i18n */ "Billing",
			link: { to: "/app/org/$org/settings/billing", params },
			icon: IconCreditCard,
		});
	}
...
function brandGroups(organization: OrganizationSummary, brand: BrandWithPrompts): NavGroup[] {
	...
	const dashboard: NavItem[] = [
		{
			title: /* i18n */ "Overview",
			link: { to: "/app/org/$org/brand/$brand", params },
			icon: IconDashboard,
			exact: true,
		},
	];
		...
		dashboard.push(
			{
				...
				link: { to: "/app/org/$org/brand/$brand/visibility", params },
				icon: IconChartBar,
			},
			{
			...
			{
				title: /* i18n */ "Query Fan-Out",
				link: { to: "/app/org/$org/brand/$brand/query-fan-out", params },
				icon: IconSitemap,
			},
			{ title: /* i18n */ "Citations", link: { to: "/app/org/$org/brand/$brand/citations", params }, icon: IconLink },
			{
				title: /* i18n */ "Opportunities",
				link: { to: "/app/org/$org/brand/$brand/opportunities", params },
				icon: IconTarget,
			},
		);
	}
	...
	if (brand.onboarded) {
		groups.push({
			...
			items: [
				{
					title: /* i18n */ "Brand",
					link: { to: "/app/org/$org/brand/$brand/settings/brand", params },
					icon: IconBuilding,
				},
				{
