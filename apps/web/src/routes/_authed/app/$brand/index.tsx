						...
						{isLoadingSummary ? (
							...
							<>
								...
								<StatWithTooltip
									icon={IconClock}
									label="run frequency"
									value={formatRunFrequency(brand?.resolvedCadenceHours ?? clientConfig?.defaultDelayHours ?? 24)}
									tooltip={`Prompts are automatically evaluated every ${formatRunFrequency(brand?.resolvedCadenceHours ?? clientConfig?.defaultDelayHours ?? 24).replace("~", "")} on average to track changes in AI model responses over time.`}
								/>
								<StatWithTooltip
									icon={IconRefresh}
									label="last updated"
