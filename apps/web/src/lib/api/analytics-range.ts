...
export function parseAnalyticsWindow(url: URL): AnalyticsWindow {
	...
	const timezone = resolveTimezone(params.get("timezone") ?? undefined, "UTC");
...
export function parsePaging(url: URL, defaultLimit = 20): { page: number; limit: number; offset: number } {
	...
	const limit = Math.max(
		1,
		Math.min(100, Number.parseInt(url.searchParams.get("limit") || String(defaultLimit), 10) || defaultLimit),
	);
