...
import { assertCanAddPromptsToOrg, assertEnableTransitionWithinPool } from "@/server/prompt-limits";
import {
	extractDomain,
	normalizeUrl,
	emptyCategoryCounts,
...
export const updatePromptsFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		const existingEnabledById = new Map(existingPrompts.map((p) => [p.id, p.enabled]));

		// Entitlement guards (§7/A5) — inert outside cloud (unlimited entitlements),
		// so non-cloud keeps exactly today's behavior (the flat MAX_PROMPTS cap
		// remains UI-level). Cloud: adding prompts checks the org pool, and flipping
		// prompts enabled false→true re-checks their Claude assignments against the
		// assignable-model pool.
		const addingCount = data.prompts.filter((p) => !p.id).length;
		await assertCanAddPromptsToOrg(brand.organizationId, addingCount);
		const enablingIds = data.prompts
			.filter((p) => p.id && p.enabled && existingEnabledById.get(p.id) === false)
			.map((p) => p.id!);
		await assertEnableTransitionWithinPool(brand.organizationId, enablingIds);
