...
export const inviteTeamMemberFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		await auth.api.createInvitation({
			body: { email: data.email, role: data.role, organizationId: organization.id },
...
export const removeTeamMemberFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		const [row] = await db
			...
			.from(member)
			.where(and(eq(member.id, data.memberId), eq(member.organizationId, organization.id)))
			.limit(1);
		...
		await auth.api.removeMember({
			body: { memberIdOrEmail: data.memberId, organizationId: organization.id },
			headers: getRequestHeaders(),
		});
