...
const log = {
	info: (msg: string) => (isCI() ? console.log(msg) : p.log.info(msg)),
	warn: (msg: string) => (isCI() ? console.warn(pc.yellow(msg)) : p.log.warn(msg)),
	error: (msg: string) => (isCI() ? console.error(pc.red(msg)) : p.log.error(msg)),
	success: (msg: string) => (isCI() ? console.log(pc.green(msg)) : p.log.success(msg)),
	step: (msg: string) => (isCI() ? console.log(msg) : p.log.step(msg)),
};
...
async function runInitInteractive(options: InitOptions, version: string): Promise<void> {
	...
	const providerChoice = await p.select({
		message: "How would you like to track AI visibility?",
		options: [
			{
				value: "olostep" as const,
				label: "Olostep (recommended)",
				hint: "Cheapest, scrapes ChatGPT, Google AI Mode, Copilot",
			},
			{
				value: "direct" as const,
				label: "Direct APIs",
				hint: "Direct calls to OpenAI, Anthropic, DataForSEO",
			},
			{
				value: "brightdata" as const,
				label: "BrightData",
				hint: "Alternative scraping service",
			},
			{
		...
		initialValue: "olostep" as const,
	});
	assertNotCancelled(providerChoice);

	if (providerChoice === "olostep") {
		...
		env.SCRAPE_TARGETS = "chatgpt:olostep:online,google-ai-mode:olostep:online,copilot:olostep:online";
	} else if (providerChoice === "direct") {
		...
		env.SCRAPE_TARGETS =
			"chatgpt:direct:gpt-5-mini:online,claude:direct:claude-sonnet-4,google-ai-mode:dataforseo:online";
	} else if (providerChoice === "brightdata") {
	...
	p.log.warn("Your .env contains secrets — do not commit it to version control.");
...
async function runInitCI(options: InitOptions, version: string): Promise<void> {
	...
	const dirArg = options.dir ?? process.env.ELMO_CONFIG_DIR ?? cwd;
	...
	if (options.dev) {
		const explicitDockerDir = options.dockerDir ?? process.env.ELMO_DOCKER_DIR;
	...
	const postgresMode: PostgresMode = (process.env.ELMO_POSTGRES_MODE as PostgresMode) ?? "docker";
	...
	if (process.env.ELMO_DATAFORSEO_LOGIN) {
		...
		env.DATAFORSEO_PASSWORD = process.env.ELMO_DATAFORSEO_PASSWORD ?? "";
	}
...
async function doStart(configDir: string): Promise<void> {
	...
	if (!isCI()) {
		...
		if (ok) {
			...
			console.warn("Some services did not report healthy status.");
		}
	}
