...
async function main() {
	...
	telemetry
		...
		.option("--dir <path>", "Config directory")
		.action(async (options: DirOption) => {
			await runTelemetryStatus(options);
		});
	...
	telemetry
		...
		.action(async (options: DirOption) => {
			...
			const envPath = await setTelemetryEnabled(configDir, true);
			log.success("Telemetry enabled.");
			log.info(`Updated ${envPath}. Restart the stack with \`elmo compose up -d\` to apply.`);
	...
	telemetry
		...
		.action(async (options: DirOption) => {
			const configDir = await resolveConfigDir(options.dir);
			const envPath = await setTelemetryEnabled(configDir, false);
...
async function runTelemetryStatus(options: DirOption): Promise<void> {
	const configDir = await resolveConfigDir(options.dir);
	const status = await getTelemetryStatus(configDir);
	...
	if (status.source === "env") {
		...
		console.log(`  Source: ${path.join(configDir, ".env")}`);
	}
	if (status.distinctId) {
		console.log(`  Deployment ID: ${status.distinctId}`);
	}
