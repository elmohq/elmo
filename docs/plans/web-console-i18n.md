...
> 状态：主体实现与静态验证完成；数据库执行及浏览器验收待明确授权  
> 编写日期：2026-08-08  
> 首批 Locale：`en`、`zh-CN`
...
- Migration 文件经过 Review，但没有在未获明确授权时执行。
...
- 在隔离的 `elmo_i18n_dev` 数据库执行现有项目 Migration。
- 启动本地 Web 服务，通过浏览器分别验证英文、简体中文、账号偏好持久化、系统默认值、刷新和 `<html lang>`。

按照仓库规则，数据库 Migration 必须获得用户单独明确授权后才能执行；该授权也是继续浏览器验收的前置条件。
