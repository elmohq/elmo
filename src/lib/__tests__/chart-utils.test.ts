...
describe("chart-utils", () => {
	...
	const createChartDataPoint = (date: string, brandVisibility: number | null, competitorVisibility: number | null = null) => ({
		date,
	...
	describe("generateDateRange", () => {
		...
		it("should not include tomorrow in the date range", () => {
			...
			const promptRuns = [createPromptRun("run-1", "2025-07-21T12:00:00.000Z", true)];
			const result = calculateVisibilityPercentages(promptRuns, mockBrand, mockCompetitors, "1w", "America/Los_Angeles");
	...
	describe("filterAndCompleteChartData", () => {
		...
		it("should return data as-is for 'all' lookback", () => {
			const inputData = [
				createChartDataPoint("2025-07-20", 50),
				createChartDataPoint("2025-07-21", 75),
			];
		...
		it("should reproduce the user's specific issue: July 21 PDT data appears as July 20 activity", () => {
			...
			promptRuns.forEach((run, i) => {
				console.log(`  Run ${i + 1}: ${run.createdAt.toISOString()} -> PDT: ${run.createdAt.toLocaleDateString("en-CA", { timeZone: "America/Los_Angeles" })}`);
			});
			
			// Step 2: calculateVisibilityPercentages buckets correctly by PDT timezone
			const visibilityData = calculateVisibilityPercentages(promptRuns, mockBrand, mockCompetitors, "1w", "America/Los_Angeles");
			
			console.log("Date range from calculateVisibilityPercentages:", visibilityData.map(d => d.date));
			...
			console.log("Date range from filterAndCompleteChartData:", chartData.map(d => d.date));
			...
			console.log(`  July 20: ${july20Data?.[mockBrand.id] !== undefined ? july20Data[mockBrand.id] + '%' : 'undefined (appears as 0% in chart)'}`);
		...
		it("should show the missing timezone parameter problem", () => {
			...
			console.log("Filtering also uses timezone-aware range:", result.map(d => d.date));
		...
		it("should demonstrate incomplete data filling", () => {
			const inputData = [
				createChartDataPoint("2025-07-21", 75, 25),
			];
	...
	describe("calculateVisibilityPercentages - Date Bucketing", () => {
		it("should bucket PDT evening events correctly", () => {
			...
			console.log("PDT date should be:", promptRuns[0].createdAt.toLocaleDateString("en-CA", { timeZone: "America/Los_Angeles" }));
			...
			console.log("Dates with data:", datesWithData.map(d => d.date));
		...
		it("should handle multiple events on same PDT date", () => {
			...
			promptRuns.forEach((run, i) => {
				...
				console.log(`Run ${i + 1} PDT date:`, run.createdAt.toLocaleDateString("en-CA", { timeZone: "America/Los_Angeles" }));
			});
			...
			const datesWithData = result.filter(point => point[mockBrand.id] !== null || point[mockCompetitors[0].id] !== null);
			console.log("Multi-event dates with data:", datesWithData.map(d => ({
				date: d.date,
		...
		it("should handle EST timezone edge cases", () => {
			...
			console.log("Run created at:", promptRuns[0].createdAt.toISOString());
			console.log("UTC date key would be:", promptRuns[0].createdAt.toISOString().split("T")[0]);
			console.log("EST date should be:", promptRuns[0].createdAt.toLocaleDateString("en-CA", { timeZone: "America/New_York" }));
