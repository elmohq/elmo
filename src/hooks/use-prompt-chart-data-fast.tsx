...
const fetcher = async (url: string): Promise<PromptChartDataFastResponse> => {
...
function buildApiUrl(brandId: string, promptId: string, filters?: PromptChartDataFilters): string {
	...
	const baseUrl = `/api/brands/${brandId}/prompts/${promptId}/chart-data-fast`;
...
export function usePromptChartDataFast(
	...
	const { data, error, isLoading, mutate } = useSWR<PromptChartDataFastResponse>(apiUrl, fetcher, {
