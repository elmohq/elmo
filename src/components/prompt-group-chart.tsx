...
export function PromptGroupChart({ 
  ...
  const groupVisibilityData = (isLoading || !brand) ? [] : calculateGroupVisibilityData(
    promptRuns || [],
    prompts,
    brand,
    competitors,
      ...
      <Card className="py-3 gap-3">
        <CardHeader className="flex justify-between items-center px-3">
          <CardTitle className="text-sm">
            {prompts[0]?.groupPrefix} <span className="text-muted-foreground">{`<`}{prompts[0]?.groupCategory?.toLowerCase()}{`>`}</span>
          </CardTitle>
