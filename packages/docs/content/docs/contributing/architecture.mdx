...
When `SCRAPE_TARGETS` is not set, `deriveLegacyDefaults()` generates equivalent configs from the legacy `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, and `DATAFORSEO_LOGIN` environment variables for backward compatibility.
