...
The `packages/lib/src/providers/` directory implements a pluggable provider system that decouples **what** Elmo tracks (models) from **how** it reaches them (providers).

### Key Concepts

| Concept | Description | Examples |
|---------|-------------|----------|
| **Model** | An AI product that end users interact with | `chatgpt`, `google-ai-mode`, `copilot`, `perplexity` |
| **Provider** | The mechanism used to query a model | `olostep`, `brightdata`, `direct`, `openrouter`, `dataforseo` |
| **Version** | A specific model version slug (required for API providers) | `gpt-5-mini`, `anthropic/claude-sonnet-4` |

### SCRAPE_TARGETS

The `SCRAPE_TARGETS` environment variable drives the entire scraping pipeline. It is parsed at startup by `parseScrapeTargets()` in `packages/lib/src/providers/config.ts` into an array of `ModelConfig` objects:
...
Providers are registered in the `providerMap` in `packages/lib/src/providers/index.ts`. The `resolveProviderId()` function handles the `direct` shorthand, mapping it to `direct-openai` or `direct-anthropic` based on the model.

### Model Registry

Known models and their display metadata are defined in `packages/lib/src/providers/models.ts`. The `KNOWN_MODELS` map provides labels and icon IDs for the UI. Unknown model strings are auto-formatted (hyphen-separated words are title-cased).
