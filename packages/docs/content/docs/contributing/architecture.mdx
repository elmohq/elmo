...
The worker iterates `SCRAPE_TARGETS` on every prompt — there are no hardcoded model lists. Adding a new provider means adding a file under `packages/lib/src/providers/registry/`, registering it in `providers/index.ts`, and teaching `packages/config/src/env.ts` which API keys it requires. The worker validates `SCRAPE_TARGETS` at startup via `validateScrapeTargets`, so misconfiguration fails fast instead of per-prompt.
