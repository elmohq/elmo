...
`competitors:write` and `competitors:delete` matter to the [REST API](/docs/api/v1) only — MCP can
...
- Self-hosting: no extra configuration, but `APP_URL` must be the address clients actually reach you
  on — that is what the sign-in flow is advertised from, and what the tokens it issues are bound to.
  It has to be an HTTPS URL, or `localhost`; the app refuses to start otherwise, because a token
  bound to anything else is one no MCP client will accept.
