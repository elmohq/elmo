...
`packages/deployment` resolves the active mode at runtime from `DEPLOYMENT_MODE` and supplies that mode's feature flags, branding, and auth hooks:

| Mode | Description |
|------|-------------|
| `local` | Single-org, email/password auth |
| `demo` | Read-only demonstration |
| `whitelabel` | Multi-org with Auth0 SSO |
| `cloud` | Managed multi-tenant, self-serve signup with Stripe billing |

## Database

The database schema is defined with Drizzle ORM in `packages/lib/src/db/`. Key tables include:

- **brands** — Brand profiles with name, domain, additional domains, aliases
