...
Optionally, `OPENAI_APPS_CHALLENGE_TOKEN` holds the token OpenAI's plugin submission portal issues to
verify the domain the MCP server is on. It is served at `<APP_URL>/.well-known/openai-apps-challenge`.
...
against a test-clock customer with `packages/deployment/scripts/verify-stripe-lifecycle.ts` (see its
header for usage).
