...
- **Grounded** (`...:online` on an API provider) is the premium tier, counted against the org's pool
  of prompt/model slots (`prompts.premium_models`) — plan included plus purchased add-on. A grounded
  call is roughly ten times an ungrounded one, which is why it is metered per prompt rather than per
  brand. Customers assign it in the prompts editor; the LLM settings page only reports the totals.
...
Claude also samples **once daily on every plan** when picked ungrounded, not at the plan's standard
rate: its API is billed per call at several times an OpenRouter one, so at Pro's 4×/day a single
Claude pick would cost more than the rest of the plan. That is the `premiumRate` flag in
`CLOUD_PLATFORMS`, and `resolvePromptRunPlan` gives such a pick its own interval.
...
Custom plans are configuration, not code. `maxBrands` is the most common override: every self-serve
plan tracks exactly one brand, so an agency or a multi-brand company needs an agreement rather than a
bigger tier.

Insert (or update) a row in `organization_settings`:
