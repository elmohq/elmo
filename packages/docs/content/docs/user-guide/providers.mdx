...
Each LLM surface defaults to a sensible model (`gpt-4o`, `sonar`, `gemini-2.5-flash`). Override the underlying DataForSEO `model_name` via the version slug, e.g. `chatgpt:dataforseo:gpt-4.1:online`. All four require `:online`.
