...
The same credentials serve two different DataForSEO products, split across two provider ids:

**`dataforseo-scraper` — the real consumer UIs (recommended for ChatGPT and Gemini).** Uses DataForSEO's [AI Optimization "LLM Scraper"](https://docs.dataforseo.com/v3/ai_optimization-chat_gpt-llm_scraper-live-advanced/) API, which drives chatgpt.com and gemini.google.com and returns what a real user sees, including product cards and tables. ChatGPT also reports its query fan-out. Only `chatgpt` and `gemini` are available — DataForSEO has no Perplexity scraper. These runs take roughly a minute since a live session has to complete.

**`dataforseo` — SERP endpoints and model APIs.**

- `google-ai-mode` uses the SERP Google AI Mode endpoint.
- `google-ai-overview` uses the Google Organic SERP endpoint with `load_async_ai_overview` on, returning the AI Overview block and its cited sources.
- `chatgpt`, `perplexity`, and `gemini` use the [AI Optimization "LLM Responses"](https://docs.dataforseo.com/v3/ai_optimization-llm_responses-overview/) API. These call the vendors' model APIs rather than the consumer products, so they're much faster and cheaper but don't reflect the real ChatGPT or Gemini experience. Prefer `dataforseo-scraper` for ChatGPT and Gemini; `perplexity:dataforseo:online` is the only way to reach Perplexity here.

Each LLM Responses surface defaults to a sensible model (`gpt-5.5`, `sonar`, `gemini-2.5-flash`). Override the underlying DataForSEO `model_name` via the version slug, e.g. `chatgpt:dataforseo:gpt-5-mini:online`. The scraper takes no version slug — the live UI picks the model. All DataForSEO targets require `:online`.
...
DATAFORSEO_LOGIN=...
DATAFORSEO_PASSWORD=...
SCRAPE_TARGETS=chatgpt:dataforseo-scraper:online,gemini:dataforseo-scraper:online,google-ai-mode:dataforseo:online
```
...
- `provider` — how to reach it (`brightdata`, `oxylabs`, `cloro`, `olostep`, `openai-api`, `anthropic-api`, `mistral-api`, `openrouter`, `dataforseo`, `dataforseo-scraper`).
- `version` — required for `openai-api`, `anthropic-api`, `mistral-api`, `openrouter`. Optional for scrapers (pass a BrightData dataset id here). Rejected for `dataforseo-scraper`.
