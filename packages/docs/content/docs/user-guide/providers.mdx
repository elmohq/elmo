...
When it doesn't, Google has loaded the overview separately from the result page, and the SERP returns `ai_overview.page_token` instead. Elmo redeems that token against `google_ai_overview` right away, since it expires within a minute, so those runs cost two searches. A run whose token still comes back without an overview fails rather than storing an empty answer, which would otherwise read as a run where nobody mentioned your brand.
