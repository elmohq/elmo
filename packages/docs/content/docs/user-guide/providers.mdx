...
On OpenRouter, `:online` runs the underlying model's own web search where one exists (Anthropic, Google, OpenAI, Perplexity, xAI) and falls back to OpenRouter's Exa search elsewhere. For Perplexity, `perplexity/sonar-pro-search` is the agentic stack perplexity.ai itself serves — $3/$15 per M tokens plus $0.018 per request. `perplexity/sonar` costs a fraction of that ($1/$1 plus $0.005) if a Perplexity-style answer is enough.
