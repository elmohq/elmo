...
<Callout>
The `SCRAPE_TARGETS` and provider-key environment variables below seed the tracked-platform catalog the first time a deployment boots. After that, you manage which platforms Elmo tracks in the app under **Admin → Config → Targets** — the environment is no longer the source of truth. Provider keys can stay in the environment (fully supported) or be stored encrypted in the app; encrypted storage requires `ELMO_ENCRYPTION_KEY` (the CLI generates one for you). Setting keys in the environment remains the simplest way to configure a fresh install.
</Callout>
...
`SCRAPE_TARGETS` seeds the tracked-platform catalog on a deployment's first boot (and drives the one-time environment → database import when you upgrade an existing install). The format below describes that seed string; once the catalog exists, targets are edited under **Admin → Config → Targets** rather than here.
