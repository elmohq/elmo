...
import { PLANS, PREMIUM_ADDON_LOOKUP_KEYS, stripePlanLookupKey } from "@workspace/config/plans";
...
if (entitlements.maxPrompts !== PLANS.pro.maxPrompts || entitlements.premiumPool !== PLANS.pro.premiumIncluded) {
...
// --- 4. Add-on quantity ------------------------------------------------------
const addonPrice = await priceByLookupKey(PREMIUM_ADDON_LOOKUP_KEYS.monthly);
await stripe.subscriptionItems.create({ subscription: subscription.id, price: addonPrice, quantity: 7 });
await waitForDb("add-on: organization_settings.premium_addon_quantity = 7", async () => {
	const [row] = await db
		.select({ premiumAddonQuantity: organizationSettings.premiumAddonQuantity })
		.from(organizationSettings)
		.where(eq(organizationSettings.organizationId, orgId))
		.limit(1);
	return row?.premiumAddonQuantity === 7 ? row : null;
});
...
await waitForDb("cancel: add-on quantity reset to 0", async () => {
	const [row] = await db
		...
		.where(eq(organizationSettings.organizationId, orgId))
		.limit(1);
	return row?.premiumAddonQuantity === 0 ? row : null;
});
